...
bool Solver::run(Klondike game) {
    ...
    while (!moves.empty() && i < maxMoves) {
        ...
        for (int j = 0; j < n; j++) {
            ...
            if (bestScore <= 100 && checkSafeMove(game, moves[j])/*moves[j].getEnd()[0] == static_cast<int>(CardLocation::FOUNDATION) && ((moves[j].getCard().getColour() == Colour::RED && moves[j].getCard().getRank() <= minFoundation[1] + 2) || (moves[j].getCard().getColour() == Colour::BLACK && moves[j].getCard().getRank() <= minFoundation[0] + 2))*/) {
            ...
            } else if (bestScore <= 20 + moves[j].getStart()[2] && revealHiddenHeur(game, moves[j], 1) > 0/*moves[j].getStart()[0] == static_cast<int>(CardLocation::TABLEAU) && moves[j].getStart()[2] > 0 && game.getTableau()[moves[j].getStart()[1]][moves[j].getStart()[2] - 1].isFaceDown()*/) {
