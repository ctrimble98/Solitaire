...
int main(int argc, char *argv[]) {
    ...
    std::cout << "Running solvers with " << deal << " deal klondike on " <<games << " games with seed " << seed << std::endl;
    ...
    comp = runGames(comp, deal, seed, games);
...
std::vector<Solver> setSolvers(std::vector<std::string> hFiles) {
    ...
    if (!hFiles.empty()) {
        ...
        solvers.push_back(Solver("Basic Solver", std::vector<Heuristic>(), runNoSearch, getDepth, 0));
    }
